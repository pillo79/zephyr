...
#define BHY2_INT_THREAD_STACK_SIZE 512
			       ...
			       enum sensor_channel chan)
{
	...
	if (gpio_pin_get_dt(&config->int_gpio)) {
		...
		LOG_DBG("%s", bhy2_get_api_error(ret));
	}
	return 0;
}
...
static int bhy2_zephyr_init(const struct device *dev)
{
	...
	ret = bhy2_init(BHY2_SPI_INTERFACE, bhy2_spi_read, bhy2_spi_write, bhy2_delay_us, MAX_READ_WRITE_LEN, (void*) dev, &data->_bhy2);
	LOG_DBG("%s", bhy2_get_api_error(ret));
	if (ret != BHY2_OK) return ret;

	bhy2_soft_reset(&data->_bhy2);
...
/* Initializes a struct bhy2_config for an instance on a SPI bus. */
#define BHY2_SPI_OPERATION (SPI_WORD_SET(8) | SPI_TRANSFER_MSB | SPI_MODE_CPOL \
		| SPI_MODE_CPHA | SPI_OP_MODE_MASTER)
#define BHY2_CONFIG_SPI(inst)				\
	{						\
		.spi = SPI_DT_SPEC_INST_GET(	\
			inst, BHY2_SPI_OPERATION, 0),	\
	}
